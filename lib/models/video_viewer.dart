...
void loadvideo() async {
    ...
    try {
      ...
      if (file == null) {
        if(kDebugMode) print('Error: Could not get the video file.');
        setState(() {
      ...
      videoFileData = file;
      if(kDebugMode) print('Loaded file data: ${videoFileData?.path}'); // Log the path
    } catch (e) {
      if(kDebugMode) print('Cannot access the video file: $e');
      setState(() {
        isVideoLoading = false;
      });
      return;
    ...
    try {
      ...
      if(kDebugMode) print('Loaded video successfully from path: ${videoFileData!.path}');

      // trying to play after a short delay
      Future.delayed(const Duration(milliseconds: 500), (){
        if(player.state.playing){
          player.play();
          if(kDebugMode) print('Attempting to play video after a short delay');
        }
      });
    } catch (e) {
      if(kDebugMode) print('Unable to open media: $e');
      setState(() {
        isVideoLoading = false;
      });
    }
